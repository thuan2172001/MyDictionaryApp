...
public class Controller {
    ...
    public void keyPressTranslate(javafx.scene.input.KeyEvent keyEvent) {
        if (keyEvent.getCode().equals(KeyCode.ENTER)) {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/translateFunction.fxml"));
                Parent root = loader.load();
    ...
    public void ShowWord(ActionEvent event) throws IOException {
        Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
        ShowWord show = new ShowWord();
        Scene scene = show.MyDictionary();
        stage.setScene(scene);
        stage.show();
    }
